...
double wittenstein_height = 0.015; // height of WITTENSTEIN sensor (1.5cm)
double honeycomb_buckling_height = 0.01; // height of buckling (1cm)
...

{
	...
	bag.open("/home/rustam/catkin_ws/src/experiments/src/new setup/Force vs Distance/test_github_hor.bag", rosbag::bagmode::Write);
	...
	geometry_msgs::Twist in_velocity_x_fast;
	in_velocity_x_fast.linear.x = 0.01;
	in_velocity_x_fast.linear.y = -0.6*0.01;
	in_velocity_x_fast.linear.z = 0;
	in_velocity_x_fast.angular.x = 0;
	in_velocity_x_fast.angular.y = 0;
	in_velocity_x_fast.angular.z = 0;
	geometry_msgs::Twist in_velocity_x_slow;
	in_velocity_x_slow.linear.x = 0.0015;
	in_velocity_x_slow.linear.y = -0.6*0.0015;
	in_velocity_x_slow.linear.z = 0;
	in_velocity_x_slow.angular.x = 0;
	in_velocity_x_slow.angular.y = 0;
	in_velocity_x_slow.angular.z = 0;
	geometry_msgs::Twist in_velocity_back_x_fast;
	in_velocity_back_x_fast.linear.x = -0.01;
	in_velocity_back_x_fast.linear.y = 0.6*0.01;
	in_velocity_back_x_fast.linear.z = 0;
	in_velocity_back_x_fast.angular.x = 0;
	in_velocity_back_x_fast.angular.y = 0;
	in_velocity_back_x_fast.angular.z = 0;
	...
	URutil Experiment;
	X_TARGET = 0.3718; // Target position for X-coordinate of end-effector
	ros::spinOnce();
	PoseShow_test.pose_now(); // getting X,Y,Z coordinates of end-effector relative to base frame
  	...
  	while((ros::ok()) && (ros::Time::now() - start_time < timeout)) {
		ROS_INFO("X_NOW before experiment: %f", X_NOW); //send the values of Z-coordinate of end-effector relative to base frame
 	}
	while( (ros::ok()) && ((X_NOW)<(X_TARGET)) )
	{
		//ROS_INFO("X_NOW: %f", X_NOW);

		if( (ros::ok()) && ((X_NOW)>(X_TARGET - 0.015)) && ((X_NOW)<(X_TARGET)) ){
			ros::Subscriber sub_force_z = nh.subscribe("/wittenstein_topic", 1000, force_z_Callback);
			ros::Subscriber sub_hallsensor = nh.subscribe("/hallsensor_teensy", 1000, &hallsensor_callback);
			PoseShow_test.pose_now();
			Experiment.send_velocity(in_velocity_x_slow);
			//ROS_INFO("I am writing message: X(%f) vs X_target(%f) and fz(%f)", global_msg.z, X_TARGET, global_msg.fz);
			//ROS_INFO("I am writing message: top(%f)", global_msg.top);

			bag.write("trial7_1", ros::Time::now(), global_msg);
		}else{
			PoseShow_test.pose_now();
			Experiment.send_velocity(in_velocity_x_fast);
		}
			ros::spinOnce();
			loop_rate.sleep();
		}
	...
	while((ros::ok()) && X_NOW > 0.307366)
		{
			PoseShow_test.pose_now();
			Experiment.send_velocity(in_velocity_back_x_fast);
			ROS_INFO("back X_NOW = %f", X_NOW);	
		}
	ROS_INFO("Finished");	
